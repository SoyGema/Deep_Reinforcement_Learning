...
dqn2 = DQNAgent(model=model, nb_actions=nb_actions, memory=memory, nb_steps_warmup=50,
              target_model_update=1e-2, policy=policy,enable_double_dqn=False, enable_dueling_network=False)
dqn2.compile(Adam(lr=1e-3), metrics=['mae', 'acc'])

import os.path

file_path = 'DQN_Taxi.h5f'
...
class Saver(Callback):
    def on_episode_end(self, episode, logs={}):
        ...
        if episode % 1 == 0:
            self.model.save_weights('DQN_Taxi.h5f', overwrite=True)


logs = FileLogger('DQN_Taxi.csv', interval = 1)
s = Saver()
dqn2.fit(env, nb_steps=20000,callbacks=[s,logs], visualize=False, verbose=2)
